...
Aplikasi Android native (Web2APK) yang membungkus website dalam sebuah WebView dengan fitur lengkap termasuk JavaScript, akses kamera untuk scanner barcode/QR code, file upload/download, dan tampilan modern Material 3 yang mendukung Android 7 (SDK 24) hingga Android 16 (SDK 36).

URL default dikonfigurasi di satu file konfigurasi (`AppConfig.kt`) sehingga mudah diganti tanpa mengubah logika aplikasi. Default URL: https://coldstorage.nexeratech.co.id

## Requirements

### Requirement 1 — WebView Core

**User Story:** As a user, I want the app to load the website https://coldstorage.nexeratech.co.id with full JavaScript support, so that all web features work correctly inside the app.

#### Acceptance Criteria

1. WHEN the app launches THEN the WebView SHALL load the default URL https://coldstorage.nexeratech.co.id
2. WHEN the WebView loads a page THEN JavaScript SHALL be enabled
3. WHEN the WebView loads a page THEN DOM Storage SHALL be enabled
4. WHEN the WebView loads a page THEN LocalStorage SHALL be enabled
5. WHEN the WebView loads a page THEN SessionStorage SHALL be enabled
6. WHEN the WebView loads a page THEN Cookies SHALL be enabled and persisted between sessions
7. WHEN JavaScript calls navigator.mediaDevices.getUserMedia() THEN the WebView SHALL support and forward the request to the system

### Requirement 2 — Camera & Barcode/QR Scanner

**User Story:** As a user, I want the app to access my device camera so that I can scan barcodes and QR codes directly from the website.
...
4. WHEN the app is installed on Android 6+ THEN camera permission SHALL be requested at runtime (not only at install)
...
3. WHEN the initial URL finishes loading THEN the splash screen SHALL transition to the main WebView screen
...
4. WHEN the status bar and navigation bar are visible THEN the WebView content SHALL render behind them (edge-to-edge)
