    ...
    <div class="flex items-center justify-between max-w-full mx-auto gap-2 md:gap-3">
        ...
        <div class="flex items-center gap-1.5 md:gap-3 flex-nowrap">
            @if($showAction('create'))
                @can('create', $model)
                <a href="{{ moduleRoute('getCreate') }}" class="inline-flex items-center justify-center gap-1 h-8 md:h-10 px-2.5 md:px-4 text-xs md:text-sm font-semibold rounded-lg bg-primary text-on-primary hover:bg-primary/90 shadow-sm transition-all active:scale-95 shrink-0">
            ...
            @if($showAction('delete'))
                @can('delete', $model)
                <x-button type="button" variant="error" icon="delete" size="xs" onclick="deleteSelected()"><span class="hidden sm:inline">Delete</span></x-button>
                @endcan
            @endif
            <a href="{{ $cancel }}" class="inline-flex items-center justify-center gap-1 h-8 md:h-10 px-2.5 md:px-4 text-xs md:text-sm font-semibold rounded-lg border border-outline-variant text-on-surface-variant hover:bg-surface-container transition-all shrink-0">
