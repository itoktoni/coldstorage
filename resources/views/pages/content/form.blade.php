...
$buildFieldChildren = function($parentId) use (&$buildFieldChildren, $fieldChildren) {
    ...
    foreach ($kids as $k) {
        $child = (object) $k;
        $child->children = $buildFieldChildren($k['id']);
        $result->push($child);
    }
    return $result;
};
$enrichField = function($field) use (&$enrichField, &$buildFieldChildren) {
    $field->children = $buildFieldChildren($field->id);
    return $field;
};
...
$selectedCategories = old('category_ids', $model->category_ids ?? []);
$selectedTags = old('tag_ids', $model->tag_ids ?? []);
