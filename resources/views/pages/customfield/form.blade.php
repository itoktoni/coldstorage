<?php /** @var App\Models\CustomField $model */ ?>
<?php
$typeOptions = \App\Models\CustomField::getTypeOptions();
...
<x-layouts::app>
    ...
    <script>
        ...
        // Build path for nested children: children[2][children][0][children]
        ...
        // Toggle container settings on type change
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if($isEdit && $model->type === 'container')
                @php
                    // Use closure to avoid redeclaration issues
                    $buildChildTree = function($children) use (&$buildChildTree) {
                        $result = [];
                        foreach ($children->sortBy('sort_order') as $c) {
                            $result[] = [
                                'id' => $c->id,
                                'label' => $c->label,
                                'name' => $c->name,
                                ...
                                'children' => $c->relationLoaded('children') ? $buildChildTree($c->children) : [],
                            ];
                        }
                        return $result;
                    };
                    $existingChildren = $buildChildTree($model->children);
                @endphp
                var existingChildren = {!! json_encode($existingChildren) !!};
